...
def plot(dx, dy, pot):
    linspc = np.linspace(0, 1, dx.shape[0])

    xx = np.outer(linspc, np.ones(dx.shape[0])).T
    print('xx: ', xx)
    print('xx shape: ', xx.shape)

    yy = np.outer(linspc, np.ones(dx.shape[0]))
    print('yy: ', yy)
...
def calc_pot(cel_robo, sonares, OCCUPANCY, recent_visited, surface):

    f = 12  # 3 células gazebo
    x_robo = cel_robo[0]
    y_robo = cel_robo[1]
    grid = np.array(OCCUPANCY)
    lim1 = x_robo-f-1 if x_robo-f-1 >= 0 else 0
    lim2 = x_robo+f-1 if x_robo+f-1 <= NEW_GRID_SIZE else NEW_GRID_SIZE
    lim3 = y_robo-f+1 if y_robo-f+1 >= 0 else y_robo-f+1
    lim4 = y_robo+f+1 if y_robo+f+1 <= NEW_GRID_SIZE else NEW_GRID_SIZE
    janela = grid[lim1:lim2, lim3:lim4]
    print('window shape: ', janela.shape)
    print('window: ', janela)

    try:
        # # calcula pot
        hpf = potential.HarmonicPotentialField(janela)
        dx, dy, pot = hpf.calc()
    except:
        dx = np.zeros(janela[0])
        dy = np.zeros(janela[0])
        pot = np.zeros(janela[0])

    # plot(dx, dy, pot)

    print('potencial: ', pot)
    print('potencial shape: ', pot.shape)

    robo_cel_relative = (int(dx.shape[0]/2), int(dy.shape[0]/2))
    print('robo_cel_relative: ', robo_cel_relative)

    return dx, dy, robo_cel_relative, pot, cel_robo
...
def draw_sonar(surface):
    ...
    dx, dy, robo_cel_relative, pot, cel_robo = calc_pot(cell_indexes_robo, SONARES, OCCUPANCY, recent_visited, surface)
    return dx, dy, robo_cel_relative, pot, cel_robo
...
def draw_pot(surface, full_pot):

    for indice_linha in range(0, len(full_pot)):
        for indice_coluna in range(0, len(full_pot[indice_linha])):
            xy = _get_xy_cell_index((indice_linha,indice_coluna))
            try:
                pygame.draw.rect(
                    surface,
                    (
                        abs((255 / 2)* full_pot[indice_linha][indice_coluna]),
                        abs((255 / 2)* full_pot[indice_linha][indice_coluna]),
                        abs((255 / 2)* full_pot[indice_linha][indice_coluna])
                    ),
                    (xy[0], xy[1],BLOCKSIZE, BLOCKSIZE),
...
def path(dx, dy, robo_cel_relative, pot, cel_robo):
    print('dx: ', dx)
    print('dx shape: ', dx.shape)
    print('dy: ', dy)
    print('dy shape: ', dy.shape)

    print('robo cel: ', robo_cel_relative)
    print('pot: ', pot)

    anguloRobo = ORIENTATION

    try:
        rdx = dx[robo_cel_relative[0]][robo_cel_relative[1]]
        rdy = dy[robo_cel_relative[0]][robo_cel_relative[1]]
        angulo_to_go = atan2(rdy, rdx)
        # print('rdx: ', rdx)
        # print('rdy: ', rdy)
    except BaseException as e:
        print('e: ', e)
        angulo_to_go = anguloRobo

    msg = Twist()
    diffang = abs(angulo_to_go - anguloRobo)
    diffang = abs(anguloRobo - angulo_to_go)
    print('anguloRobo: ', anguloRobo)
    print('angulo_to_go: ', angulo_to_go)
    print('diff: ', diffang)
    msg.linear.x = 0
    if diffang > 0.2: 
        msg.angular.z = 0.1
        ...
        msg.angular.z = 0
        msg.linear.x = 0.2
        print("acertou o angulo")
    PUBLISHER.publish(msg)

    '''
    # OBJETIVO
    result_vetor_x = rdx - cel_robo[0]
    result_vetor_y = rdy - cel_robo[1]
    try:
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        dx, dy, robo_cel_relative, pot, cel_robo = draw_sonar(surface)
        path(dx, dy, robo_cel_relative, pot, cel_robo)
