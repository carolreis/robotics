...
def prob_bayes(R,r,B,a, prob_atual):
    psh = abs(1/2 * (((R-r)/R)  + ((B-a)/B)))
    ph = prob_atual
    '''
            P(s|H) * P(h) 
    P(s|H) * P(h) + P(s|¬H) * P(¬h)
    '''
    bayes = (psh * ph) / ( (psh*ph) + ((1-psh) * (1-ph)) + 0.0000001)
    return bayes
    ...
    for sonar in SONARES:
        ...
        if len(sonar.items()) > 0:
            ...
            for r in [r/20 for r in range(0,60)]:
                ...
                # Divide cada metade do ângulo de abertura em 2
                for b in range(1,20):

                    # Faz para cada metade do cone
                    for a in [sonar['angle'] - (angulo_visao/b), sonar['angle'] + (angulo_visao/b)]:
                        ...
                        if (xy_indexes) not in cells and sonar['range'] < (MAX_RANGE - 0.3):

                            # Adiciona célula na lista
                            cells.append(xy_indexes)

                            R = sonar['range']  # Range do gazebo
                            angulo_sonar = sonar['angle']  # Ângulo do sonar considerando a posição do robô (do gazebo)
                            B = angulo_visao  # Angulo (fixo) de visão do robô

                            # Faz o cálculo apenas para células que tem raio menor que o raio de leitura + 0.3
                            if r < (R + 0.5):

                                # Obtém angulo entre célula e eixo acústico
                                resulting_angle = abs(angulo_sonar - a)

                                # Probabilidade atual da célula = 
                                print('xy: ', xy)
                                print('xy_indexes: ', xy_indexes)
                                print('tam occupancy: ', len(OCCUPANCY))

                                prob_atual = OCCUPANCY[xy_indexes[0]][xy_indexes[1]]

                                # Calcula probabilidade
                                bayes = prob_bayes(R,r,B,resulting_angle, prob_atual)

                                if bayes > 1:
                                    bayes = 1

                                # Monta grade de ocupação
                                # TODO: Por que está invertido ?????
                                if r < (R - 0.5):
                                    OCCUPANCY[xy_indexes[0]][xy_indexes[1]] = 1 - bayes
                                else:
                                    OCCUPANCY[xy_indexes[0]][xy_indexes[1]] = bayes

                                print('PROB: %s, raio: %s' % (bayes,r))
    ...
    for indice_linha in range(0, len(OCCUPANCY)):
        for indice_coluna in range(0, len(OCCUPANCY[indice_linha])):
            ...
            pygame.draw.rect(
                surface,
                (
                    # abs(255 * OCCUPANCY[indice_linha][indice_coluna] - 255),
                    # abs(255 * OCCUPANCY[indice_linha][indice_coluna] - 255),
                    # abs(255 * OCCUPANCY[indice_linha][indice_coluna] - 255),
                    255 * (OCCUPANCY[indice_linha][indice_coluna]),
                    255 * (OCCUPANCY[indice_linha][indice_coluna]),
                    255 * (OCCUPANCY[indice_linha][indice_coluna])
                ),
